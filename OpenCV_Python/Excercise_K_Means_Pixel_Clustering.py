...
def main(opt) :
    ...
    for imageName in imageNameList :
        ...
        # Load Image
        image = imageRead(imagePath) # H x W X 3
    
        # Flatten Image Spatial-wise
        data = image.reshape(-1,3).astype(np.float32) # HW x 3
        
        # 반복 중지 요건
        criteria = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 10, 1.0)

        # 평균 클러스터링 적용
        retval, bestLabels, centers = cv2.kmeans(data, K=3, bestLabels=None, criteria=criteria, attempts=10, flags=cv2.KMEANS_RANDOM_CENTERS)

        # Get Centers of Different Clusters
        centers = centers.astype(np.uint8)
        
        # Reshape Image
        dst = centers[bestLabels].reshape(image.shape) # HW x 3 -> H x W x 3
